...
export default function ScheduledMonitoringDate({ schedule, value, onChange, readonly }) {
  ...
  return <div className="sm-field"><label htmlFor="monitoringDate">Monitoring date</label>
    ...
    <p id="monitoring-schedule-help">{!schedule.configured ? UNSCHEDULED : !schedule.dates.length ? 'No eligible dates in this window. Contact your Supervisor.' : `Due: ${displayDate(schedule.setting?.due_date)}. Select from the published eligible dates.`}</p>
