...
export default function SupperMonitoringPage({ location, employee, managerPin, supervisorPin, onBack }) {
  ...
  return <div className="login-app sm-app"><header className="login-header"><div className="login-brand"><div className="login-logo spark-login-logo"><img src="/spark-192.png" alt="SPARK" /></div><div><div className="login-brand-name">SOUTH CAFÉ LA</div><div className="login-brand-subtitle">SUPPER MONITORING</div></div></div><button type="button" disabled={busy} onClick={() => { if (data) leaveEditor(); else run(async () => { if (token) await closeSession(token); onBack(); }); }}>{data ? (readonly ? "Return to Monitorings" : "Save & Return to Monitorings") : supervisorPin ? "← School Overview" : "← Manager Hub"}</button></header>
