...
export default function SupperMonitoringPage({ location, employee, managerPin, supervisorPin, onBack }) {
  ...
  function renderSection() {
    if (section <= 4) return <GuidedSections {...{ section, data, change, field, Field, errors, showErrors, countOptions, historyWeek, location, actionFields }} slots={Object.fromEntries(Object.entries(SLOTS).filter(([slot]) => (slot === "supervisor") === ((record?.monitor_role || context?.actor_role) === "supervisor")))} />;
