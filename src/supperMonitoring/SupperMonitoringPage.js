...
import { editableGuided, SLOTS, STATUSES } from "./workflow";
import SignaturePad from "./SignaturePad";
import { SECTIONS, DAYS, OFFICIAL_FORM, newDraft, weekDates, average, schoolYear, validate, applicableQuestions, findings } from "./model";
import { openSession, openSupervisorSession, getContext, closeSession, listMonitorings, getMonitoring, saveDraft, submitMonitoring, downloadReport } from "./service";
...
export default function SupperMonitoringPage({ location, employee, managerPin, supervisorPin, onBack }) {
  ...
  function openRecord(loaded) {
    setRecord(loaded); setData({ ...newDraft(), ...loaded.payload, schoolYear: loaded.school_year, monitoringSlot: loaded.monitoring_slot });
  ...
  function renderSection() {
    if (section === 0) return <><p>Confirm the date and the time you arrived and left. Your school is fixed to your authorized location.</p><p><strong>{location.school_name}</strong> · {location.location_code}</p><div className="sm-grid">{<Field id="monitoringDate" label="Monitoring date" type="date" value={data.monitoringDate} errors={errors} reveal={showErrors} onChange={value => { change("monitoringDate", value); change("schoolYear", schoolYear(value) || data.schoolYear); }} />}{field("arrivalTime", "Arrival time", { type: "time" })}{field("departureTime", "Departure time", { type: "time" })}</div>{choices("unannounced", "Was this visit unannounced?", data.unannounced, value => change("unannounced", value))}<div className="sm-grid">{field("schoolYear", "School year", { placeholder: "2026-27" })}<label>Monitoring slot<select value={data.monitoringSlot || ""} onChange={e => change("monitoringSlot", e.target.value)}><option value="" disabled>Choose a slot</option>{Object.entries(SLOTS).filter(([slot]) => (slot === "supervisor") === ((record?.monitor_role || context?.actor_role) === "supervisor")).map(([slot,label]) => <option key={slot} value={slot}>{label}</option>)}</select></label></div></>;
    ...
    if (section === 7) return <><p>Each signer should confirm their printed name, sign, and explicitly accept applying the signature to both official pages.</p>{["monitor", "coordinator"].map(role => <section className="sm-subcard" key={role}><h3>{role === "monitor" ? ((record?.monitor_role || context?.actor_role) === "supervisor" ? "Supervisor / AFSS" : "Manager / Monitor") : "ASP Coordinator"}</h3>{field(`${role}Name`, `${role === "monitor" ? ((record?.monitor_role || context?.actor_role) === "supervisor" ? "Supervisor / AFSS" : "Manager / Monitor") : "ASP Coordinator"} printed name`)}<SignaturePad key={`${record?.id}-${role}-${data.signatures[role]?.acceptedAt || "unsigned"}`} label={role === "monitor" ? ((record?.monitor_role || context?.actor_role) === "supervisor" ? "Supervisor / AFSS" : "Manager / Monitor") : "ASP Coordinator"} name={data[`${role}Name`]} value={data.signatures[role]} disabled={readonly} onChange={value => acceptSignature(role, value)} />{visibleErrors.filter(e => e.field === `${role}Signature`).map((e, i) => <p className="sm-error" key={i}>{e.message}</p>)}</section>)}</>;
    if (section === 8) return <><p>Review each section. Select an item to see its details or fix a problem.</p><ul className="sm-review">{SECTIONS.slice(0, 8).map((label, i) => { const issues = errors.filter(e => e.section === i); return <li key={label}><button type="button" onClick={() => navigate(i)}>{issues.length ? "○" : "✓"} {label}<span>{issues.length ? `${issues.length} item(s) to review` : "Complete"}</span></button>{issues.length > 0 && <p>{issues[0].message}</p>}</li>; })}</ul><p><strong>School:</strong> {location.school_name}<br /><strong>Monitoring:</strong> {data.monitoringDate || "Not entered"}<br /><strong>Comments:</strong> {data.comments || "Not entered"}</p>{readonly && <><p>This monitoring is read-only. Status: {STATUSES[record.status]}.</p><button type="button" onClick={() => reportAction("download")}>Download Official PDF</button></>}</>;
  ...
  return <div className="login-app sm-app"><header className="login-header"><div className="login-brand"><div className="login-logo spark-login-logo"><img src="/spark-192.png" alt="SPARK" /></div><div><div className="login-brand-name">SOUTH CAFÉ LA</div><div className="login-brand-subtitle">SUPPER MONITORING</div></div></div><button type="button" disabled={busy || reviewActive} onClick={() => { if (data) leaveEditor(); else run(async () => { if (token) await closeSession(token); onBack(); }); }}>{data ? (readonly ? "Return to Monitorings" : "Save & Return to Monitorings") : supervisorPin ? "← School Overview" : "← Manager Hub"}</button></header>
    ...
    {!token ? <section className="sm-card sm-unlock"><h2>Opening your school's monitorings</h2><p>{busy ? "Connecting…" : "Connect using your current SPARK sign-in."}</p>{!busy && <button type="button" onClick={unlock}>Retry Connection</button>}</section> : !data ? <MonitoringHome onReviewActive={setReviewActive} token={token} records={records} context={context} location={location} onRefresh={refresh} onOpen={openRecord} onStart={(year,slot) => { setRecord(null); setData({ ...newDraft(supervisorPin ? "" : employee.employee_name, context.actor_role), schoolYear: year, monitoringSlot: slot }); setSection(0); setDirty(true); setShowErrors(false); }} /> : <>
    ...
    <div className="sm-actions sm-bottom"><button type="button" disabled={busy || section === 0} onClick={() => navigate(section - 1)}>← Previous</button>{section < 9 && <button className="sm-primary" type="button" disabled={busy} onClick={() => navigate(section + 1)}>{readonly ? "Next →" : "Save & Continue →"}</button>}</div><p className="sm-save-status">{readonly ? "" : dirty ? "Changes have not been saved yet." : "Draft saved."} {readonly ? "This record is read-only until returned or unlocked for correction." : "Save Draft and Save & Continue preserve your work and current section."}</p>
    </>}</main></div>;
}
