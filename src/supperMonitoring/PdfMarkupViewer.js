...
import { rotatePoint, rotateAnnotations } from '../monitoring/pdfRotation';
...
export default function PdfMarkupViewer({ bytes, annotations, onChange, editable=false, onReady, rotations={}, onRotate }) {
  ...
  useEffect(() => {
    ...
    setDocument(null);setError('');setReady(false);setMarker(null);
    (async()=>{
      ...
      if(!cancelled){setPage(p=>Math.min(p,loaded.numPages));setDocument(loaded);}
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      if(cancelled)return;
      const original=pdfPage.getViewport({scale:1,rotation:pdfPage.rotate+rotation});
      const viewport=pdfPage.getViewport({scale:700*zoom/original.width,rotation:pdfPage.rotate+rotation});
  ...
  },[document,page,zoom,onReady,rotation]);
  ...
  function pointerUp(event){if(!drawing.current || drawing.current.id!==event.pointerId)return;const points=drawing.current.points;drawing.current=null;setStroke([]);if(points.length>1)onChange([...annotations,{type:'draw',page,points:points.map(p=>rotatePoint(p,-rotation))}]);}
  ...
  return <div className="sm-pdf-viewer">
    <div className="sm-actions sm-pdf-toolbar">
      ...
      {onRotate && <><button type="button" disabled={!ready} onClick={()=>onRotate(page,270)}>Rotate Page Left</button><button type="button" disabled={!ready} onClick={()=>onRotate(page,90)}>Rotate Page Right</button></>}
    </div>
