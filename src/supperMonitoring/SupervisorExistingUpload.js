...
export default function SupervisorExistingUpload({ schools, supervisorPin, initialYear, onBack, onUploaded }) {
  ...
  const [metadata,setMetadata]=useState({schoolYear:initialYear,monitoringSlot:'manager_1',monitoringDate:'',managerEmployeeId:'',monitorName:'',onBehalf:true,monitoringType:'supper',monitoringSiteId:''});
  ...
  const occupied=slot=>records.some(r=>r.status!=='deleted'&&r.monitoring_site_id===metadata.monitoringSiteId&&(r.monitoring_type||'supper')===metadata.monitoringType&&r.school_year===metadata.schoolYear&&r.monitoring_slot===slot);
