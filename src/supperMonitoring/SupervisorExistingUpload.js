...
import { supperSequence } from '../monitoring/supperSchedule';
...
export default function SupervisorExistingUpload({ schools, supervisorPin, initialYear, onBack, onUploaded }) {
  ...
  const blocked=slot=>metadata.monitoringType==='supper' ? supperSequence(records,metadata.monitoringSiteId,metadata.schoolYear,slot) : '';
  ...
  return <section className="sm-card"><h2>Upload Existing Monitoring</h2>
    ...
    <fieldset className="sm-editor" disabled={busy}>
      ...
      {metadata.monitoringType==='supper' ? <label>Monitoring slot<select value={metadata.monitoringSlot} onChange={e=>setMetadata({...metadata,monitoringSlot:e.target.value})}>{['manager_1','manager_2'].map(slot=><option key={slot} value={slot} disabled={occupied(slot) || !!blocked(slot)}>{SLOTS[slot]}{blocked(slot)?' — locked':''}{occupied(slot)?' — already exists':''}</option>)}</select></label> : <><label>Monitoring number<input type="number" min="1" step="1" value={metadata.monitoringNumber} onChange={e=>setMetadata({...metadata,monitoringNumber:e.target.value})}/></label><label>Performed by<select value={metadata.performerRole} onChange={e=>setMetadata({...metadata,performerRole:e.target.value})}><option value="manager">Manager</option><option value="supervisor">Supervisor / AFSS</option></select></label></>}
      ...
      {blocked(metadata.monitoringSlot) && <p>🔒 {blocked(metadata.monitoringSlot)}</p>}<PdfUploadPicker files={files} onChange={setFiles} disabled={busy}/>
