...
export default function MysterySupervisor({token,onExpired}) {
 ...
 return <div className="mystery-admin"><header className="mystery-school-heading"><div><p className="mystery-eyebrow">SUPERVISOR AREA</p><h1>Mystery Pull Control Room</h1><p>Tokens, prizes, and deliveries. SPARK-point prizes credit the winning school automatically.</p></div><button className="mystery-secondary" onClick={reload} disabled={busy}>Refresh</button></header>
  ...
  {data&&tab==='prizes'&&<section><div className="mystery-section-title"><h2>Prize inventory</h2><button className="mystery-primary" disabled={disabled} onClick={()=>setEditor(emptyPrize())}>Add a prize</button></div><p>Each active prize with stock has an equal chance. Add an extra pull to a prize to award both together. Automatic rewards are delivered immediately.</p>
   {editor&&<div className="mystery-editor"><form className="mystery-admin-form mystery-prize-fields" onSubmit={e=>{e.preventDefault();save({action:'prize',...editor});}}><h3>{editor.id?'Edit prize':'New prize'}</h3>
    ...
    <label>Fulfillment<select value={editor.kind} onChange={e=>setEditor({...editor,kind:e.target.value,points_amount:e.target.value==='spark_points'?editor.points_amount:0,bonus_tokens:e.target.value==='extra_pull'?0:editor.bonus_tokens})}><option value="manual">Manual · Huy fulfills</option><option value="extra_pull">Automatic · +1 Mystery Token</option><option value="spark_points">Automatic · SPARK points for school</option></select></label>
    {editor.kind==='spark_points'&&<label>SPARK points awarded<input type="number" required min="1" max="1000000" step="1" value={editor.points_amount??0} onChange={e=>setEditor({...editor,points_amount:e.target.value===''?'':Number(e.target.value)})}/><small>Added to the winning school’s SPARK total immediately.</small></label>}
    {editor.kind!=='extra_pull'&&<label className="mystery-checkbox"><input type="checkbox" checked={editor.bonus_tokens===1} onChange={e=>setEditor({...editor,bonus_tokens:e.target.checked?1:0})}/> Include 1 extra pull with this prize</label>}
   ...
   <div className="mystery-prize-grid">{data.prizes.map(p=><article className="mystery-prize-card" key={p.id}><span aria-hidden="true">{p.icon}</span><div><h3>{p.name}</h3><p>{p.description}</p>{p.points_amount>0&&<p>+{p.points_amount} SPARK points for the winning school</p>}{p.bonus_tokens>0&&<p>Includes 1 extra pull</p>}<strong className="mystery-status">{p.inventory===0?'OUT OF STOCK':`${p.inventory} remaining`} · {p.active?'Active':'Inactive'}</strong><button className="mystery-secondary" disabled={disabled} onClick={()=>{setEditor({...p});setStock('1');}}>Edit {p.name}</button></div></article>)}</div>
  </section>}
