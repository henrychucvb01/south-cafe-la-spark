...
export default function MysteryManager({token,onExpired}) {
 ...
 const [fruit]=useState(()=>{const choices=['🍋','🍊','🍎','🍓','🍍'];let last;try{last=sessionStorage.getItem('spark-mystery-fruit');}catch{}const available=choices.filter(f=>f!==last);const selected=available[Math.floor(Math.random()*available.length)];try{sessionStorage.setItem('spark-mystery-fruit',selected);}catch{}return selected;});
