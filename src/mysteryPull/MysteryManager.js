 ...
 return <>
  ...
  <section ref={stage} className={`mystery-machine ${phase}`} aria-label="Mystery Pull machine">
   <div className="mystery-prize-stage" aria-live="polite" aria-atomic="true">
    ...
    :<><div className="mystery-spinner-scene" aria-hidden="true"><div className="mystery-swirl mystery-swirl-one"/><div className="mystery-swirl mystery-swirl-two"/><div className="mystery-swirl mystery-swirl-three"/><div className="mystery-fruit-orbit">{['🍋','🍊','🍎','🍓','🍍'].map((fruit,index)=><div className="mystery-fruit-position" key={fruit} style={{'--fruit-angle':`${index*72}deg`}}><span>{fruit}</span></div>)}</div><div className="mystery-capsule"><span className="mystery-spin-border"/><span className="mystery-spin-icon">✦</span></div></div><h2>{phase==='drawing'?'Something good is on its way…':phase==='recover'?'Let’s check your pull':'A little mystery. A big reveal.'}</h2><p>{phase==='drawing'?'Finding your surprise…':phase==='recover'?'Your request is saved. Continue it safely without starting a second pull.':'Your next surprise is one full pull away.'}</p></>}
   </div>
