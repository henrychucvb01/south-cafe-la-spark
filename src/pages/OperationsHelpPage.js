...
function OperationsHelpPage({ location, onBack }) {
  ...
  return <div className="manager-resources-page">
    ...
    <main className="manager-resources-main">
      ...
      <section className="dashboard-card" style={{ padding: 20 }}><h2 style={{ marginTop: 0 }}>Still not sure? Call your CPM.</h2>{locationRecord?.cpm_name ? <><p><strong>{locationRecord.cpm_name}</strong> · CPM for {locationRecord.school_name}</p>{locationRecord.cpm_phone && <p><strong>Office:</strong> {locationRecord.cpm_phone}</p>}{locationRecord.cpm_cell_phone && <p><strong>Cell:</strong> {locationRecord.cpm_cell_phone}</p>}{locationRecord.cpm_email && <p><strong>Email:</strong> {locationRecord.cpm_email}</p>}</> : <p>Your CPM contact is available in Location Information. For a Food Services operational issue that does not fit above, contact your AFSS.</p>}</section>
    </main>
  </div>;
}
