  ...
  return (
    <article className={`location-info-details ${ownLocation ? "own-location" : ""}`}>
      ...
      <dl className="location-info-grid">
        ...
        <div><dt>Complex Project Manager (CPM)</dt><dd>{record.cpm_name || "Not listed"}</dd></div>
        <div><dt>CPM office phone</dt><dd><ContactLink value={record.cpm_phone} /></dd></div>
        <div><dt>CPM cell phone</dt><dd><ContactLink value={record.cpm_cell_phone} /></dd></div>
        <div><dt>CPM email</dt><dd><ContactLink value={record.cpm_email} type="email" /></dd></div>
      </dl>
    </article>
  );
}
...
export function SchoolPhoto({ record }) {
  ...
  return <div className={`location-map-photo ${hasPhoto ? "has-photo" : ""}`} aria-label={hasPhoto ? `${record.school_name} school photo` : "School photo placeholder"}>{hasPhoto ? <img src={record.school_photo_url} alt={`${record.school_name} campus`} onError={() => setFailedUrl(record.school_photo_url)} /> : <><span aria-hidden="true">▧</span><p>School photo coming soon</p></>}</div>;
}
...
function MapLocationCard({ record }) {
  if (!record) return <aside className="location-map-listing location-map-listing-empty"><p>Select a school marker to view its location information.</p></aside>;
  return <aside className="location-map-listing" aria-live="polite" aria-label={`Selected school: ${record.school_name}`}>
    <SchoolPhoto key={record.id} record={record} />
    <div className="location-map-listing-copy">
      <div className="location-map-listing-heading"><ListingThumbnail key={record.id} record={record} /><div className="location-map-listing-title"><h2>{record.school_name}</h2><p>Location {record.location_code || "not assigned"}</p></div></div>
      <div className={`location-map-address ${record.address ? "" : "is-empty"}`}><span aria-hidden="true">⌖</span><div><strong>Address</strong><p>{record.address || "Address not currently listed"}</p></div></div>
      <dl className="location-map-contact-list"><div><dt>Cafeteria Phone</dt><dd><ContactLink value={record.cafeteria_phone} /></dd></div><div><dt>School Phone</dt><dd><ContactLink value={record.school_phone} /></dd></div><div><dt>CPM</dt><dd>{record.cpm_name || "Not listed"}</dd></div><div><dt>CPM Phone</dt><dd><ContactLink value={record.cpm_phone || record.cpm_cell_phone} /></dd></div></dl>
    </div>
  </aside>;
}
...
function LocationInformationPage({ location, onBack }) {
  ...
  useEffect(() => {
    ...
    Promise.all([loadLocationDirectory(), loadAreaSupervisor()])
      .then(([data, areaSupervisor]) => { if (!active) return; setRecords(data); setSupervisor(areaSupervisor); const own = data.find((item) => String(item.location_code) === String(location?.location_code)); setSelectedId(own?.id || data[0]?.id || null); })
  ...
  return <div className="manager-resources-page location-info-page">
    <header className="login-header"><div className="login-brand"><div className="login-logo spark-login-logo"><img src="/spark-192.png" alt="SPARK" /></div><div><div className="login-brand-name">SOUTH CAFÉ LA</div><div className="login-brand-subtitle">LOCATION INFORMATION</div></div></div><button type="button" className="homebase-exit-button" onClick={onBack}>← Manager Resources</button></header>
    <main className="location-info-main">
      ...
      {!loading && !error && <><AreaSupervisorCard supervisor={supervisor} />{ownLocation ? <LocationDetails record={ownLocation} ownLocation /> : <div className="location-info-state">Your location is not yet connected to the area directory. You can still browse all locations below.</div>}<section className="location-directory-section"><div className="location-directory-heading"><div><span>ALL LOCATIONS</span><h2>Browse the area</h2></div><div className="location-directory-controls"><div className="location-view-toggle" role="group" aria-label="Choose directory or map view"><button type="button" className={view === "directory" ? "active" : ""} aria-pressed={view === "directory"} onClick={() => setView("directory")}>☷ Directory</button><button type="button" className={view === "map" ? "active" : ""} aria-pressed={view === "map"} onClick={() => setView("map")}>⌖ Map</button></div>{view === "directory" && <label><span className="sr-only">Search locations</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search school, code, manager, CPM, or type" /></label>}</div></div>{view === "directory" ? <div className="location-directory-layout"><div className="location-directory-list" role="list" aria-label="Area locations">{filtered.map((record) => <button type="button" role="listitem" key={record.id} className={record.id === selectedId ? "selected" : ""} onClick={() => setSelectedId(record.id)}><span><strong>{record.school_name}</strong><small>{record.location_code ? `Location ${record.location_code}` : "Location code not assigned"}</small></span><span>{record.site_type?.toLocaleUpperCase() || "—"}</span></button>)}{!filtered.length && <p className="location-directory-empty">No locations match “{query}”.</p>}</div><LocationDetails record={selected} /></div> : <div className="location-map-layout"><LocationMap records={records} selectedId={selectedId} onSelect={setSelectedId} /><MapLocationCard record={selected} /></div>}</section></>}
    </main>
  </div>;
}
