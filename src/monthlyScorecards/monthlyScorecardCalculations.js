...
function laborCostForMonth(school,dataset,operatingDays,laborRows){
  const staffing=(dataset.staffing||[]).filter((row)=>String(row.source_site_id)===String(school.source_site_id));
  const rates=new Map((dataset.labor_rates||[]).map((row)=>[row.classification_key,n(row.hourly_rate)]));
  if(!staffing.length||!rates.size)return{total:null,scheduled:null,adjustments:null,filledDailyHours:null,method:null};
  const manager=staffing.find((row)=>String(row.classification_key).startsWith("manager_")&&n(row.filled_count)>0);
  const missingRate=staffing.some((row)=>n(row.filled_daily_hours)>0&&!rates.has(row.classification_key));
  if(!manager||missingRate)return{total:null,scheduled:null,adjustments:null,filledDailyHours:null,method:"PWI staffing is incomplete for labor-cost calculation."};
  const filledDailyHours=staffing.reduce((sum,row)=>sum+n(row.filled_daily_hours),0);
  const scheduledDailyCost=staffing.reduce((sum,row)=>sum+n(row.filled_daily_hours)*n(rates.get(row.classification_key)),0);
  const scheduled=scheduledDailyCost*operatingDays;
  const extraWorkerHours=laborRows.reduce((sum,row)=>sum+n(row.additional_worker_hours),0);
  const managerExtraHours=laborRows.reduce((sum,row)=>sum+n(row.manager_overtime_hours),0);
  const adjustments=extraWorkerHours*n(rates.get("worker"))+managerExtraHours*n(rates.get(manager.classification_key));
  return{total:scheduled+adjustments,scheduled,adjustments,filledDailyHours,method:"Estimated wages from filled PWI scheduled hours plus recorded labor adjustments at supplied straight-time hourly rates; excludes benefits and any overtime premium."};
}
...
function calculateMonth(school,dataset,month){
  ...
  const monthLaborRows=(dataset.labor_hours||[]).filter((row)=>String(row.location_id)===String(school.location_id)&&inMonth(row.service_date,month));
  const laborByDate=new Map(monthLaborRows.map((row)=>[row.service_date,row]));
