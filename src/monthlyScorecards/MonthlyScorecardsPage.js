...
const REPORT_ORDER = ["production","cost","official_meal_count"];
...
export default function MonthlyScorecardsPage({ supervisorPin }) {
  ...
  async function upload(reportType,file) {
    ...
    try {
      ...
      setMessage(`${REPORT_TYPES[reportType]} imported: ${parsed.normalizedRows.length} normalized rows, ${parsed.rejectedRows.length} rejected.`);
      await refresh();
  ...
  return <section className="monthly-scorecards" aria-labelledby="monthly-scorecards-title">
    <div className="monthly-scorecards-heading">
      <div><span className="monthly-kicker">MONTHLY OPERATIONS</span><h3 id="monthly-scorecards-title">Monthly Scorecards</h3><p>Validate and store the three official monthly reports. Uploading a corrected report safely replaces that report for the selected month.</p></div>
    ...
    <div className="monthly-import-grid">
      {REPORT_ORDER.map((type) => {
        ...
        return <article className="monthly-import-card" key={type}>
          <div className="monthly-import-title"><span aria-hidden="true">{type==="production"?"📋":type==="cost"?"💵":"✓"}</span><div><h4>{REPORT_TYPES[type]}</h4><p>{type==="production"?"Menu and production detail":type==="cost"?"Breakfast, Lunch, and Supper food cost":"Official Main Site meal verification"}</p></div></div>
          <div className={`monthly-status ${batch?.status || "missing"}`}>{batch ? batch.status : "Not uploaded"}</div>
          {batch ? <dl className="monthly-import-facts"><div><dt>File</dt><dd>{batch.original_filename}</dd></div><div><dt>Imported</dt><dd>{batch.imported_row_count.toLocaleString()} rows</dd></div><div><dt>Rejected</dt><dd>{batch.rejected_row_count.toLocaleString()} rows</dd></div><div><dt>Uploaded</dt><dd>{new Date(batch.uploaded_at).toLocaleString()}</dd></div></dl> : <p className="monthly-empty">No report stored for this month.</p>}
    ...
    <div className="monthly-safety-note"><strong>Import safety</strong><span>SPARK checks the report type, required columns, reporting month, site IDs, and supported meals before writing. Snack and Extra Sales are excluded. Manager-entered counts are retained and reconciled separately.</span></div>
  </section>;
}
