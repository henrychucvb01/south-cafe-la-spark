...
export function createHandler({ database, templateLoader = () => readFile(resolve(process.cwd(), "public/supper-monitoring-2022-09-08.pdf")) }) {
  return async (request, response) => {
    ...
    const { action, id, revision } = request.body || {};
    if (!["preview", "submit", "download"].includes(action) || !/^[a-f0-9-]{36}$/.test(id || "")) return response.status(400).json({ error: "Choose a saved monitoring and a valid action." });
    ...
    try {
      ...
      if (action === "download") return response.status(409).json({ error: "This monitoring has not been submitted. Preview the draft instead." });
      ...
      const bytes = await generateOfficialPdf(template, record.payload, school);
      ...
      const status = error.section !== undefined ? 422 : /Session expired|access|assigned|not found for this school/i.test(error.message) ? 403 : /changed|revision/i.test(error.message) ? 409 : 500;
