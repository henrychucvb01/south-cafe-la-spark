...
import {getLocalDateString,isSubmissionOnTime,SCHOOL_YEAR_START} from '../src/sparkPolicy.js';
...
const schools=locations.filter(s=>s.school_name.trim().toLowerCase()!=='test high school').map(s=>{
 ...
 for(const monday=day('2026-08-17');;monday.setUTCDate(monday.getUTCDate()+7)){
  const friday=new Date(monday);friday.setUTCDate(friday.getUTCDate()+4);const end=keyDate(friday);if(end>today)break;
...
lines.push(``,`## Banning HS`,``,...schools.find(s=>s.school_name==='Banning HS').bonus.map(b=>`- ${b.date}: ${b.kind}, expected ${b.points}, recorded ${b.actual}, missing ${b.missing}.`),``,`## Code findings`,``,`The live app still uses August 12 as REWARD_LAUNCH_DATE, contrary to the confirmed September 3 rollout. Bonuses are attempted when School Dashboard opens and failures are not shown. Historical and current bonus keys differ, allowing repeated awards for the same period. These are implementation findings only; no production changes or point adjustments were made.`);
