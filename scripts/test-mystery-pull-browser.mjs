...
try{
 ...
 await page.goto(origin);await page.getByRole('button',{name:'✦ Mystery Pull',exact:true}).click();await enter('0928');await expect(page.getByRole('heading',{name:'Mystery Pull Control Room'})).toBeVisible();
 ...
 await dragTo(1.1);await expect(page.getByRole('heading',{name:'Something good is on its way…'})).toBeVisible();await expect(page.locator('.mystery-prize-stage h2')).toHaveText('Candy Bag',{timeout:12000});await expect(page.locator('.mystery-token-count strong')).toHaveText('2');
 ...
 await page.getByRole('button',{name:'Sign out',exact:true}).click();await enter('0928');await page.getByRole('button',{name:'Prizes & history',exact:true}).click();await expect(page.locator('.mystery-admin')).toContainText('Candy Bag');await page.getByRole('button',{name:'Mark received / fulfilled'}).click();await expect(page.getByText('No prizes waiting for fulfillment.')).toBeVisible();
