...
try {
 ...
 const setting=slot=>({p_pin:'9999',p_year:'2026-27',p_slot:slot,p_due:'2027-05-09',p_revision:null});
 await assert.rejects(()=>rpc('save_supper_due_date',{...setting('supervisor'),p_pin:'1234'}),/Supervisor authorization/);
 await assert.rejects(()=>rpc('save_supper_due_date',{...setting('supervisor'),p_due:'2027-07-01'}),/selected school year/);
 const existing=(await rpc('supper_context',{p_token:manager})).supper_schedules[0];
 await rpc('save_supper_due_date',{...setting('manager_1'),p_revision:existing.revision});
 for(const slot of ['supervisor','manager_2'])await rpc('save_supper_due_date',setting(slot));
 await assert.rejects(()=>rpc('save_supper_due_date',setting('supervisor')),/Schedule changed/);
 const schedules=(await rpc('supper_context',{p_token:manager})).supper_schedules;
 assert.equal(schedules.length,3);assert.ok(schedules.every(s=>!('monitoring_site_id' in s) && !('available_start' in s) && !('available_end' in s)));
 ...
 for(const date of ['2026-09-01','2027-05-18'])await assert.rejects(()=>save('manager_2',third,main.id,date),/unavailable/);
 await rpc('save_supper_due_date',{...setting('manager_2'),p_due:'2026-09-29',p_revision:1});
 await db.exec('reset role');assert.equal((await db.query("select * from supper_eligible_dates($1,'2026-27','manager_2')",[main.id])).rows.length,0);
 await rpc('save_supper_due_date',{...setting('manager_2'),p_revision:2});
